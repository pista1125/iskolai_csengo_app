...
class SchoolBellApp {
  ...
  private setupEventListeners() {
    ...
    const resetConfigLogic = () => {
      if (confirm('Biztosan törlöd a Firebase beállításokat és újra konfigurálod?')) {
        localStorage.removeItem('firebase-bell-config');
        window.location.reload();
      }
    };
    document.getElementById('edit-config-btn')?.addEventListener('click', resetConfigLogic);
    document.getElementById('register-edit-config-btn')?.addEventListener('click', resetConfigLogic);

    document.getElementById('login-btn')?.addEventListener('click', () => this.handleLogin());
    document.getElementById('register-submit-btn')?.addEventListener('click', () => this.handleRegister());
    
    // Switch between login & register screen listeners
    ...
    document.getElementById('google-register-btn')?.addEventListener('click', () => this.handleGoogleLogin());

    document.getElementById('logout-btn')?.addEventListener('click', () => {
      if (firebaseAuth) signOut(firebaseAuth);
    });

    // Setup password field eye-toggles
  ...
  private async handleGoogleLogin() {
    ...
    try {
      const googleBtns = [
        ...
        document.getElementById('google-register-btn') as HTMLButtonElement
      ];
      googleBtns.forEach(btn => {
        if (btn) {
          btn.disabled = true;
          btn.textContent = 'KAPCSOLÓDÁS...';
        }
      });
      ...
      if (e.code !== 'auth/popup-closed-by-user') {
        alert('Sikertelen Google bejelentkezés: ' + this.getHungarianAuthError(e.code));
      }
    } finally {
      const googleBtns = [
        { id: 'google-login-btn', text: 'BEJELENTKEZÉS GOOGLE-LEL' },
        { id: 'google-register-btn', text: 'BEJELENTKEZÉS GOOGLE-LEL' }
      ];
      googleBtns.forEach(item => {
  ...
  private setupPasswordToggles() {
    const toggles = [
      { btnId: 'toggle-login-password', inputId: 'auth-password' },
      { btnId: 'toggle-reg-password', inputId: 'reg-password' },
      { btnId: 'toggle-reg-password-confirm', inputId: 'reg-password-confirm' }
    ];
